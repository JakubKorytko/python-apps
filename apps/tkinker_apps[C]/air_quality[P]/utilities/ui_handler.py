...
class UIHandler:
    ...
    def select_city(self, selected):
        ...
        for index, station in enumerate(self.menu.stations):
            self.draw("city")(
                self.menu.canvas, station, {"row": index, "columnspan": 1}
            )
            ...
            button = self.draw("station_button")(
                ...
                index,
                self.menu.radio_var,
                lambda arg=index: self.generate(arg),
            )
    ...
    def generate(self, station_id=0):
        ...
        self.draw("city")(
            self.app.canvas, station["stationName"], {"row": 0, "columnspan": 2}
        )

        if self.menu:
            self.draw("select_station")(
                self.app.canvas, lambda: self.select_city(station_id)
            )
