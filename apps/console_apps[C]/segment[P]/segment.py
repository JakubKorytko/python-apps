...
def point_is_on_the_segment(point, segment):
    ...
    x_axis = (point["x"] >= min(segment["x_a"], segment["x_b"])
              and point["x"] <= max(segment["x_a"], segment["x_b"]))

    y_axis = (point["y"] >= min(segment["y_a"], segment["y_b"])
              and point["y"] <= max(segment["y_a"], segment["y_b"]))
